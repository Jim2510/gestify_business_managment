    ...
    return (
        <div className="flex flex-col w-[100vw] h-[100vh] text-center justify-center items-center mt-[30px] bg-gradient-to-b from-[#1cdafe] to-[#02f9ae]">
            ...
            <div className="w-[85%] h-[75%] overflow-y-auto">
                <div className="grid grid-cols-8 grid-rows-26 w-full h-fit shadow-lg rounded-md bg-white">
                    ...
                    {currentWeek.map((day, index) => (
                        <div key={index} className="grid justify-items-center items-center border">{day}</div>
                    ))}
                    {hours && hours.map((hour, index) => <div key={index} className="grid col-start-1 col-end-2 justify-items-center items-center border">{hour}</div>)}
                </div>
